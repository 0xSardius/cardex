...
| Database | PostgreSQL via Supabase (+ pgvector for fuzzy search) |
...
- **Append-only price data.** PricePoints are immutable; MarketSnapshots are daily aggregations.
...
│   │   ├── api/v1/           # x402-gated API endpoints (Lucid Agents middleware)
│   │   │   ├── price/        # Card price lookup
│   │   │   ├── arbitrage/    # Cross-platform arbitrage scan
│   │   │   ├── grade/        # Vision-based grading estimate
│   │   │   ├── portfolio/    # Portfolio valuation
│   │   │   └── set/          # Set completion advisor
│   │   ├── (dashboard)/      # Human-facing dashboard pages
│   │   └── layout.tsx
│   ├── lib/
...
- Use `src/` directory with App Router
- Server Components by default; `"use client"` only when needed
- All API responses include `agent` field with ERC-8004 Solana address + reputation
- Environment variables prefixed: `SUPABASE_`, `UPSTASH_`, `SOLANA_`, `ANTHROPIC_`
...
5. Minimal Next.js dashboard (search + card detail + price chart)
