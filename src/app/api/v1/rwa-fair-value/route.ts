...
export async function POST(request: NextRequest) {
  ...
  // 3. Paper price aggregation. For graded mints, prefer matching-grade
  //    price_points (e.g. condition='psa-8' from Pokemon Price Tracker);
  //    fall back to raw TCGPlayer/CardMarket when no graded data is on hand.
  //    Flagged via condition_basis so bots can filter mismatched comparisons.
  const targetCondition = gradedConditionFor(mintRow);
  let paperPrice: {
  ...
  if (collectible) {
    paperPrice = await fetchPaperPrice(sql, collectible.id, targetCondition);
  }
 ...
 *
 * Supported graders (matching Pokemon Price Tracker + future CGC/BGS adapters):
 *   PSA  → psa-1..psa-10
 *   CGC  → cgc-1..cgc-10 (also cgc-8.5, cgc-9.5 — half grades pass through)
 *   BGS  → bgs-1..bgs-10
 *   SGC  → sgc-1..sgc-10
 */
function gradedConditionFor(mintRow: {
  grader: string | null;
  grade: string | null;
}): string | null {
  ...
  // Whole grade -> integer suffix (psa-8). Half grade -> decimal preserved (cgc-9.5).
  const suffix = Number.isInteger(numeric)
    ? `${numeric}`
    : numeric.toFixed(1);
...
/**
 * Pull paper price for a card. Strategy:
 *   1. If a graded condition is requested, look for matching-grade rows
 *      observed in the last 30 days. If we have any, return graded basis.
 *   2. Otherwise fall back to raw rows in the last 7 days.
 *
 * 30-day window for graded prices because Pokemon Price Tracker free tier
 * refreshes a small subset per day — a card's PSA points may be older.
 */
async function fetchPaperPrice(
  ...
  // Raw fallback (or the mint was raw to begin with).
  const rows = (await sql`
    SELECT
      (percentile_cont(0.5) WITHIN GROUP (ORDER BY price_usd::numeric))::text AS median_usd,
      COUNT(DISTINCT source)::text AS source_count,
  ...
  return {
    ...
    fresh_minutes: p.newest_observed_at
      ? Math.floor((Date.now() - new Date(p.newest_observed_at).getTime()) / 60_000)
      : null,
    // If a graded condition was requested but no graded data existed, flag
    // explicitly so consumers know the spread compares grades inconsistently.
